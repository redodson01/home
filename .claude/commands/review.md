...
Start by resolving the base ref. If no base ref was provided, use the remote's default branch from `git symbolic-ref --short refs/remotes/origin/HEAD`, or else `origin/main` or `origin/master`, whichever exists. Then inspect the diff: `git diff <base-ref>...HEAD` (three-dot, from the merge-base: the branch's own changes, not divergence caused by the base advancing after you branched).
...
Tag each finding that proposes adding or changing code with its question-4 class as well, for example `[non-blocking][elective]`. A finding that only reports a defect in existing work carries no scope tag.
